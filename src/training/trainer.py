...
class PhysicalWindLoss(nn.Module):
    """Physical-consistency loss for wind super-resolution.
    
    Incorporates physics-informed constraints for wind data:
    - Wind speed consistency (magnitude preservation)
    - Spatial smoothness (adjacent cells should be similar)
    - Energy conservation
    
    Reference: NREL sup3r physics-informed training
    """
    ...
    def _compute_wind_speed(self, wind: torch.Tensor) -> torch.Tensor:
        """Compute wind speed magnitude from u/v components.
        
        Args:
            wind: Tensor of shape (B, C, H, W) where C >= 2
                  Assumes channel 0 = u (zonal), channel 1 = v (meridional)
        """
        u = wind[:, 0:1, :, :]
        v = wind[:, 1:2, :, :]
        return torch.sqrt(u ** 2 + v ** 2 + 1e-8)
    ...
    def forward(self, sr: torch.Tensor, hr: torch.Tensor) -> torch.Tensor:
        ...
        sr_speed = self._compute_wind_speed(sr)
        hr_speed = self._compute_wind_speed(hr)
        losses["speed"] = F.mse_loss(sr_speed, hr_speed)
        
        # 2. Spatial smoothness (reduce noise)
        sr_grad_x, sr_grad_y = self._compute_gradient(sr[:, :2, :, :])  # Use first 2 channels (u, v)
        hr_grad_x, hr_grad_y = self._compute_gradient(hr[:, :2, :, :])
        
        smoothness_sr = torch.mean(torch.abs(sr_grad_x) + torch.abs(sr_grad_y))
        smoothness_hr = torch.mean(torch.abs(hr_grad_x) + torch.abs(hr_grad_y))
        losses["smoothness"] = F.mse_loss(smoothness_sr, smoothness_hr)
        
        # 3. Divergence-free constraint (for incompressible flow)
        # div(V) = du/dx + dv/dy ≈ 0
        div_sr = sr_grad_x[:, 0:1, :, :] + sr_grad_y[:, 1:2, :, :]
        div_hr = hr_grad_x[:, 0:1, :, :] + hr_grad_y[:, 1:2, :, :]
        losses["divergence"] = F.mse_loss(div_sr, div_hr)
        
        # Weighted total
        total = (
            self.speed_weight * losses["speed"] +
            self.smoothness_weight * losses["smoothness"] +
            self.divergence_weight * losses["divergence"]
        )
...
class SRLoss(nn.Module):
    ...
    Now includes optional physics-informed loss for wind data.
    """
    ...
    def __init__(
        ...
        physical_weight: float = 0.0,  # New: physics-informed loss
        device: str = "cuda",
    ):
        super().__init__()
        ...
        # Physics-informed loss for wind data
        if physical_weight > 0:
            self.physical_loss = PhysicalWindLoss(
                speed_weight=0.5,
    ...
    def forward(
        ...
        # Physical loss (wind-specific)
        if self.physical_loss is not None:
            losses["physical"] = self.physical_loss(sr, hr) * self.physical_weight
